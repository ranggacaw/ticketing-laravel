...
<body class="bg-app text-app-primary">
    <div
        class="fixed top-0 left-0 w-full h-full -z-10 bg-[radial-gradient(circle_at_0%_0%,rgba(99,102,241,0.05),transparent_40%),radial-gradient(circle_at_100%_100%,rgba(168,85,247,0.05),transparent_40%)] dark:bg-[radial-gradient(circle_at_0%_0%,rgba(99,102,241,0.12),transparent_40%),radial-gradient(circle_at_100%_100%,rgba(168,85,247,0.12),transparent_40%)]">
    </div>
    ...
    <div class="flex min-h-screen relative">
        ...
        <!-- Sidebar -->
        <aside
            class="sidebar w-64 fixed inset-y-0 left-0 z-50 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out">
            <div class="flex flex-col h-full p-6">
                ...
                <nav class="flex-1 space-y-2">
                    @if(auth()->user()->role === 'admin' || auth()->user()->role === 'staff')
                        ...
                        <a href="{{ route('admin.history.index') }}"
                            class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.history.*') ? 'bg-indigo-50 dark:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-slate-200' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if(auth()->user()->role === 'admin')
                        <a href="{{ route('admin.users.index') }}"
                            class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.users.*') ? 'bg-indigo-50 dark:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-slate-200' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('scan') }}"
                        class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('scan') ? 'bg-indigo-50 dark:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-slate-200' }} mt-auto">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="mt-auto px-4 py-4 glass rounded-2xl border border-indigo-100 dark:border-indigo-500/10">
                    <p class="text-[10px] text-slate-500 dark:text-slate-500 uppercase tracking-widest font-bold mb-1">
                        Signed in as</p>
                    <p class="text-sm font-bold text-slate-900 dark:text-slate-200 truncate">{{ auth()->user()->name }}
                    </p>
                    <p
                        class="text-[10px] text-indigo-500 dark:text-indigo-400 font-bold uppercase tracking-widest mt-0.5">
                        {{ auth()->user()->role }}</p>

                    <a href="{{ route('my.history') }}"
                        class="flex items-center gap-2 mt-3 px-3 py-2 rounded-xl text-xs font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-900/5 dark:hover:bg-white/5 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors border border-transparent hover:border-black/5 dark:hover:border-white/5">
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="mt-2">
                        @csrf
                        <button type="submit"
                            class="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 text-xs font-bold transition-all duration-200 border border-rose-500/10">
            ...
            <header class="h-16 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-40">
                <div class="lg:hidden">
                    <button id="sidebar-toggle"
                        class="p-2 -ml-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-white/5">
                ...
                <div class="flex items-center gap-2 sm:gap-4 ml-auto">
                    <!-- Theme Switcher -->
                    <div
                        class="flex items-center gap-1 glass rounded-full p-1 border border-slate-200 dark:border-white/5 shadow-sm">
                        <button onclick="setTheme('light')" id="theme-light"
                            class="w-8 h-8 flex items-center justify-center rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-indigo-600 dark:hover:text-white transition-all theme-btn cursor-pointer"
                            title="Light Theme">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
