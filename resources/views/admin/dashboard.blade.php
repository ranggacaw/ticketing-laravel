...
<div class="space-y-8 animate-in fade-in duration-700">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h2 class="text-3xl font-bold tracking-tight">Admin <span class="gradient-text">Dashboard</span></h2>
            <p class="text-slate-400 mt-1 font-light text-sm">Comprehensive overview of ticket sales and guest activity.</p>
        </div>
        <div class="flex items-center gap-3">
            <div class="glass-card px-6 py-3 rounded-2xl border-indigo-500/20 flex flex-col items-center">
                <span class="text-xs text-indigo-400 uppercase tracking-widest font-bold">Total Sales</span>
                <span class="text-xl font-bold text-indigo-200">Rp {{ number_format($totalSales, 0, ',', '.') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- Total Tickets -->
        <div class="glass-card p-6 rounded-3xl border-white/5 group hover:border-indigo-500/30 transition-all duration-500">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-indigo-400/50">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M5.293 7.707a1 1 0 010-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414L11 5.414V17a1 1 0 11-2 0V5.414L6.707 7.707a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                    </svg>
                </span>
            </div>
            <h4 class="text-slate-400 text-sm font-medium uppercase tracking-wider">Total Tickets</h4>
        ...
        <!-- Validated Tickets -->
        <div class="glass-card p-6 rounded-3xl border-white/5 group hover:border-emerald-500/30 transition-all duration-500">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="bg-emerald-500/10 text-emerald-400 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider">Scanned</span>
            </div>
            <h4 class="text-slate-400 text-sm font-medium uppercase tracking-wider">Validated</h4>
            <div class="flex items-baseline gap-2 mt-1">
                <span class="text-3xl font-bold text-white">{{ $validatedTickets }}</span>
                <span class="text-xs text-slate-500">({{ $totalTickets > 0 ? round(($validatedTickets/$totalTickets)*100) : 0 }}%)</span>
            </div>
        </div>

        <!-- Unvalidated Tickets -->
        <div class="glass-card p-6 rounded-3xl border-white/5 group hover:border-amber-500/30 transition-all duration-500">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-amber-500/10 rounded-2xl flex items-center justify-center text-amber-400 group-hover:scale-110 transition-transform">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
                <span class="bg-amber-500/10 text-amber-400 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider">Pending</span>
            </div>
            <h4 class="text-slate-400 text-sm font-medium uppercase tracking-wider">Unvalidated</h4>
            <div class="flex items-baseline gap-2 mt-1">
                <span class="text-3xl font-bold text-white">{{ $unvalidatedTickets }}</span>
                <span class="text-xs text-slate-500">Remaining</span>
            </div>
        </div>

        <!-- Sales (Small Version or something else) -->
        <div class="glass-card p-6 rounded-3xl border-white/5 group hover:border-violet-500/30 transition-all duration-500">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-violet-500/10 rounded-2xl flex items-center justify-center text-violet-400 group-hover:scale-110 transition-transform">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
                <span class="text-violet-400/50">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                    </svg>
                </span>
            </div>
            <h4 class="text-slate-400 text-sm font-medium uppercase tracking-wider">Revenue</h4>
            <div class="flex items-baseline gap-2 mt-1">
                <span class="text-2xl font-bold text-white leading-tight">Rp {{ number_format($totalSales, 0, ',', '.') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mt-6">
        <!-- VVIP -->
        <div class="glass p-5 rounded-3xl border-white/5 hover:bg-white/5 transition-all">
            ...
            <p class="text-2xl font-bold text-slate-200">{{ $vvipTickets }}</p>
        </div>
        ...
        <div class="glass p-5 rounded-3xl border-white/5 hover:bg-white/5 transition-all">
            ...
            <p class="text-2xl font-bold text-slate-200">{{ $vipTickets }}</p>
        </div>
        ...
        <div class="glass p-5 rounded-3xl border-white/5 hover:bg-white/5 transition-all">
            ...
            <p class="text-2xl font-bold text-slate-200">{{ $festivalTickets }}</p>
        </div>
        ...
        <div class="glass p-5 rounded-3xl border-white/5 hover:bg-white/5 transition-all">
            ...
            <p class="text-2xl font-bold text-slate-200">{{ $gaTickets }}</p>
        </div>
    </div>
    ...
    <div class="glass-card rounded-3xl overflow-hidden border-white/10">
        <div class="p-6 border-b border-white/5 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-slate-200">Recent Tickets</h3>
            <a href="{{ route('admin.tickets.index') }}" class="text-sm text-indigo-400 hover:text-indigo-300 font-medium transition-colors">Manage All &rarr;</a>
        </div>
