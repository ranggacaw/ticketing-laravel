...
<div class="max-w-6xl mx-auto animate-in fade-in duration-700">
    <div class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            <a href="{{ route('admin.tickets.index') }}" class="flex items-center gap-2 text-slate-400 hover:text-slate-200 transition-colors text-sm font-medium mb-2">
        ...
        <div class="flex items-center gap-3">
             <a href="{{ route('admin.tickets.edit', $ticket) }}" class="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/10 text-slate-300 hover:bg-white/5 transition-all text-sm font-semibold">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                </svg>
                Edit
            </a>
            <a href="{{ route('admin.tickets.export', $ticket) }}" class="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white transition-all text-sm font-bold shadow-lg shadow-indigo-600/20">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <dl class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                        <div>
                            ...
                            <dd class="text-lg font-black text-indigo-400 font-mono tracking-tighter">{{ $ticket->seat_number }}</dd>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <dd>
                                @if($ticket->scanned_at)
                                    ...
                                    <p class="text-[10px] text-slate-500 mt-2">Scanned on: {{ $ticket->scanned_at->format('M d, Y H:i:s') }}</p>
                                @else
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/5 text-slate-500 text-xs font-black uppercase tracking-wider border border-white/10">
                                        Pending Scan
                                    </span>
                                @endif
                            </dd>
        ...
        <div class="lg:col-span-2 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="glass-card rounded-3xl p-8 border-white/10 flex flex-col items-center">
                    <h3 class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-8 self-start">Matrix (QR) Format</h3>
                ...
                <!-- 1D Barcode -->
                <div class="glass-card rounded-3xl p-8 border-white/10 flex flex-col items-center">
                    <h3 class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-8 self-start">Linear (1D) Format</h3>
                    <div class="barcode-preview bg-white w-full py-10 px-6 rounded-2xl flex flex-col items-center justify-center border-4 border-white/10 shadow-2xl shadow-white/5 overflow-hidden">
                        <div class="barcode-svg-container">
                            {!! $barcode128 !!}
                        </div>
                        <div class="mt-4 text-[10px] font-mono text-slate-500 font-medium tracking-widest">{{ $ticket->barcode_data }}</div>
                    </div>
                    <p class="mt-8 text-xs text-slate-500 text-center font-light leading-relaxed">
            ...
            <div class="glass-card rounded-3xl p-6 border-indigo-500/20 bg-indigo-500/5 flex items-center gap-6">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-1">This ticket can be scanned using the front-end simulation interface to test entry flow.</p>
                </div>
            </div>
        </div>
    </div>
